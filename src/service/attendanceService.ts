...
//   }));
// };
//LAST WORKING IMPLEMENTATION (THU MAR 26 10:19AM)
// import axios from "axios";
...
export const getEventItems = async (): Promise<EventItem[]> => {
  const events = await getAttendanceEvents();
  return events.map((event) => ({
    id: event.attendanceId.toString(),
    name: event.name,
    logType: event.logType,      // ← THIS WAS MISSING
    semester: event.semester,    // ← THIS WAS MISSING
    status: determineEventStatus(event),
    time: formatEventTime(event)
  }));
};
